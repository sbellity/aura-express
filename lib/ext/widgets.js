  ...
  return function(env) {
    ...
    function parseWidgetOptions(el) {
      ...
      core.util.each(data, function(k,v) {
        var key = core.util.decamelize(k).toLowerCase().replace(/^aura_/, '');
        if (key !== 'widget') {
          options[key] = v;
        } else {
          var ref = v.split("@");
      ...
      var widgetsPath    = env.config.widgets.sources[widgetSource] || "widgets";
    ...
    return {
      ...
      init: function(env) {
        ...
        env.core.registerWidgetType = function(type, def) {
